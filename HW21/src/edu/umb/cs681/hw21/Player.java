...
public class Player {
	...
	public static void main(String args[]) {
		...
		int totalGoals = players.stream().parallel().map((Player p) -> p.getGoals())
				.reduce((result, fat) -> result += fat).get();
		System.out.println("totalGoals: " + totalGoals);

		// highest rating player
		Player highestRatedPlayer = players.stream().parallel().max(Comparator.comparing(Player::getRating)).get();
		System.out.println("highestRatedPlayer: " + highestRatedPlayer.getPlayerName() + " | " + "Rating: "
				+ highestRatedPlayer.rating);

		// Lowest rating player
		Player lowestRatedPlayer = players.stream().parallel().min(Comparator.comparing(Player::getRating)).get();
		System.out.println("lowestRatedPlayer: " + lowestRatedPlayer.getPlayerName() + " | " + "Rating: "
				+ lowestRatedPlayer.rating);

		// Maximum number of goals
		Player highestGoalsByPlayer = players.stream().parallel().max(Comparator.comparing(Player::getGoals)).get();
		System.out.println("highestGoalsByPlayer: " + highestGoalsByPlayer.getPlayerName() + " | " + "Goals: "
				+ highestGoalsByPlayer.goals);

		// Minimum number of goals
		Player lowestGoalsByPlayer = players.stream().parallel().min(Comparator.comparing(Player::getGoals)).get();
		System.out.println("lowestGoalsByPlayer: " + lowestGoalsByPlayer.getPlayerName() + " | " + "Goals: "
				+ lowestGoalsByPlayer.goals);

		// Sorting by rating (Ascending)
		List<Player> sortedByRating = players.stream()
                .parallel()
		...
		List<Player> sortedByGoals = players.stream()
                .parallel()
                .sorted(Comparator.comparing(Player::getGoals, Comparator.reverseOrder()))
                .collect(Collectors.toList());
		System.out.println("Players ordered by goals (Descending): ");
