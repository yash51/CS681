...
public class Car {
	...
	private int price, gasCapacity;

	public Car(String make, String model, int mileage, int year, int price, int gasCapacity) {
		super();
		this.model = model;
		this.make = make;
	...
	public static void setUp(ArrayList<Car> cars) {
		cars.add(new Car("Tata", "Nano", 200, 2015, 2000, 100));
		cars.add(new Car("Tesla", "3", 30, 2019, 500, 400));
		cars.add(new Car("BMW", "Q5", 300, 2012, 20000, 800));
		cars.add(new Car("Mercedes", "Benz", 0, 2020, 300, 1200));
		cars.add(new Car("Ford", "Escatic", 10, 2018, 800, 1000));
	...
	static Integer minPrice(ArrayList<Car> cars) {
		return cars.stream().parallel().map((Car car) -> car.getPrice()).reduce(0, (result, price) -> {
			if (result == 0)
				return price;
	...
	static Integer maxPrice(ArrayList<Car> cars) {
		return cars.stream().parallel().map((Car car) -> car.getPrice()).reduce(0, (result, carPrice) -> {
			if (result == 0)
				return carPrice;
	...
	static Integer totalGasCapacity(ArrayList<Car> cars) {
		return cars.stream().parallel().map(Car::getGasCapacity).reduce(0, (result, hp) -> result += hp,
				(finalResult, intermediateResult) -> finalResult + intermediateResult);

	}

	static Integer countOfCars(ArrayList<Car> cars) {
		return cars.stream().parallel().map((Car car) -> car.getMake()).reduce(0, (result, make) -> ++result,
				(finalResult, intermediateResult) -> finalResult + intermediateResult);
	}
	...
	public static void main(String args[]) {
		...
		Integer totalGasCapacity = totalGasCapacity(cars);

		System.out.println("Number of cars is: " + totalGasCapacity);
